        ...
        element-loading-background="rgba(0, 0, 0, 0.8)" border>
            ...
            <el-table-column label="模型名称" prop="model_name">
                <template slot-scope="scope">
                    <el-button type="text" @click="consumptionModelDetail(scope.row)">{{scope.row.model_name}}</el-button>
                </template>
            </el-table-column>
            <el-table-column label="创建人" prop="creator"></el-table-column>
            <!-- <el-table-column label="客户数量" prop="amount"></el-table-column> -->
            <el-table-column label="定制服务费(元)" prop="model_fee">
                 <template slot-scope="scope">
                    <span>{{scope.row.model_fee}}</span>
            ...
            <el-table-column label="创建日期" prop="create_time">
                <template slot-scope="scope">
                    <span>{{scope.row.create_time | date(true)}}</span>
                </template>
            </el-table-column>
    ...
    export default {
        ...
        methods:{
            ...
            getTableList(){
                ...
                this.$http.get(this.$api.platform.modelConfigList,{params:params}).then(res=>{
                    if(res.data.code===0){
            ...
            consumptionModelDetail(row){
                let modelId=row.id
                this.$router.push({path:'/consumptionModelDetail',query:{modelId:modelId}})
            }
        },
        created(){
            this.getTableList()
